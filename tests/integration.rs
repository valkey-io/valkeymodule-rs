...
fn test_non_blocking_auth_callbacks() -> Result<()> {
    ...
    setup_acl_users(&mut con, &users)?;

    // Test successful authentications
    for (user, pass) in users.iter() {
        check_auth(&mut con, user, pass, AuthExpectedResult::Success)?;
    }
...
fn test_multiple_inflight_blocking_auth_callbacks() -> Result<()> {
    ...
    let users_auth1 = [
        ("blockUser1", "blockPass1"),
        ("blockUser2", "blockPass2"),
        ("blockUser3", "blockPass3"),
    ];
    ...
    // Create all users with correct passwords
    setup_acl_users(&mut setup_con, &users_auth1)?;
    setup_acl_users(&mut setup_con, &users_auth2)?;
    ...
    let auth1_handles: Vec<_> = users_auth1
        ...
        .map(|(user, _pass)| {
            let mut con = get_valkey_connection(port)
                ...
                .unwrap();
            let user = user.to_string();
            let wrong_pass = "wrong".to_string(); // Using "wrong" as in other tests
...
#[test]
fn test_blocking_auth_callbacks_with_disconnect() -> Result<()> {
    let port = 6516;
    let _guards =
        vec![start_valkey_server_with_module("auth", port)
            .with_context(|| FAILED_TO_START_SERVER)?];
    ...
    let auth_handle = std::thread::spawn(move || {
        let (user, pass) = users[0]; // Using the first (and only) user
        let result: RedisResult<String> = redis::cmd("AUTH").arg(&[user, pass]).query(&mut con);
    ...
    let mut verify_con =
        get_valkey_connection(port).with_context(|| FAILED_TO_CONNECT_TO_SERVER)?;
    let (user, pass) = users[0];
    check_auth(&mut verify_con, user, pass, AuthExpectedResult::Success)?;
