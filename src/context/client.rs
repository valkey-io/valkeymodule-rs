use crate::{
    ...
    RedisModule_SetClientNameById, ValkeyString,
};
...
impl Context {
    ...
    pub fn set_client_name_by_id(&self, client_id: u64, client_name: &ValkeyString) -> i64 {
    ...
    /// wrapper for RedisModule_DeauthenticateAndCloseClient
    pub fn deauthenticate_and_close_client_by_id(&self, client_id: u64) {
        unsafe {
            if let Some(deauth_fn) = RedisModule_DeauthenticateAndCloseClient {
                deauth_fn(self.get_raw(), client_id);
            }
        }
    }

    /// wrapper for RedisModule_DeauthenticateAndCloseClient using current client ID
    pub fn deauthenticate_and_close_client(&self) {
