...
unsafe extern "C" fn auth_reply_wrapper<T: 'static + Clone>(
    ...
    if module_private_data.is_null() {
        panic!("[auth_reply_wrapper] Private data is null; this should not happen!");
    }

    let user_private_data = &*(module_private_data as *const BlockedClientPrivateData<T>);

    match &user_private_data.reply_callback {
        Some(ReplyCallback::Auth(cb)) => {
            let data = user_private_data
                .data
                .as_ref()
                .map(|boxed| (**boxed).clone());
            match cb(&context, username, password, data) {
                Ok(status) => status,
                Err(error) => {
...
unsafe extern "C" fn free_callback_wrapper<T: 'static>(
    ...
    if module_private_data.is_null() {
        panic!("[free_callback_wrapper] Private data is null; this should not happen!");
    }

    let user_private_data = Box::from_raw(module_private_data as *mut BlockedClientPrivateData<T>);

    // If free_callback exists, we expect data to exist as well due to our API design.
    // The free_callback is used to clean up any resources associated with the data.
    if let Some(free_cb) = user_private_data.free_callback {
        if let Some(data) = user_private_data.data {
            free_cb(&context, *data);
        } else {
            panic!("[free_callback_wrapper] Data is null but free callback exists!");
        }
    }
}
