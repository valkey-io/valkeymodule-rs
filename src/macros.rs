...
/// Requires Redis 7.2+ or Valkey 8.0+.
#[macro_export]
                ...
                paste::paste! {
                    extern "C" fn [<__do_auth_ $module_name _ $auth_callback>](
                        ...
                        match $auth_callback(&context, username, password) {
                            Ok(result) => result,
                            Err(e) => {
                                let error_msg = $crate::ValkeyString::create(Some(ctx_ptr), e.to_string().as_str());
                    ...
                    unsafe {
                        $crate::raw::RedisModule_RegisterAuthCallback.expect("RedisModule_RegisterAuthCallback should exist on Redis 7.2 and above")($ctx, Some([<__do_auth_ $module_name _ $auth_callback>]));
                    }
                }
            }
        )*
