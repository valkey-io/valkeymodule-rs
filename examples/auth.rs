...
use valkey_module::raw::reply_with_error;
use valkey_module::{
...
fn auth_callback_one(
    ...
    authenticate_user_common(ctx, &username, &password, "user1", "pass1", "auth_one")
}

// Authentication callback for user2
//
// Similar to `auth_callback_one` but handles user2/pass2 credentials.
...
fn auth_callback_two(
    ...
    authenticate_user_common(ctx, &username, &password, "user2", "pass2", "auth_two")
}
...
// Reply callback for blocked clients in blocking_auth_callback_one
//
...
// * `priv_data` - Private data associated with the authentication context
//
// # Returns
...
// Reply callback for blocked clients in blocking_auth_callback_two
//
...
fn blocking_auth_common(
    ...
    if callback_name_str == "blocking_auth_callback_two" {
        if username_str == "blockAbort" && password_str == "abort" {
            ...
            reply_with_error(
                ctx_ptr,
                "ABORT: Authentication aborted by server\0".as_ptr() as *const c_char,
            );
    ...
    std::thread::spawn(move || {
        // Simulate authentication blocking behavior with a 2-second delay in
        // case of blocking_auth_callback_two handler
        if callback_name_str == "blocking_auth_callback_two" {
...
//   * username: "blockUser1", password: "blockPass1"
//   * username: "blockUser2", password: "blockPass2"
//   * username: "blockUser3", password: "blockPass3"
...
fn blocking_auth_callback_one(
    ...
    blocking_auth_common(
        ...
        |user, pass| match (user, pass) {
            ("blockUser1", "blockPass1") => AuthResult::Allow,
            ("blockUser2", "blockPass2") => AuthResult::Allow,
            ("blockUser3", "blockPass3") => AuthResult::Allow,
...
// # Authentication Rules
// - Allowed combinations:
//   * username: "blockUser4", password: "blockPass4"
//   * username: "blockUser5", password: "blockPass5"
...
fn blocking_auth_callback_two(
    ...
    blocking_auth_common(
        ...
        |user, pass| match (user, pass) {
            ("blockUser4", "blockPass4") => AuthResult::Allow,
            ("blockUser5", "blockPass5") => AuthResult::Allow,
            ("blockUser6", "blockPass6") => AuthResult::Allow,
            ("blockUser4", _) | ("blockUser5", _) | ("blockUser6", _) => AuthResult::Deny,
            _ => AuthResult::Next,
        },
